...
会社が AI の知識をつけろと従業員に言っていたのもあり、AI エージェントの作成やら LLM など AI に関して何か始めてみようかと思っていた。最近聞いたのが LangChain というもので、これについて学んでみたいと思っていたが何なのかすらもわかってない状況であったため調査してみた。今回は Langchain が何なのかという概要や紹介、また Langchain を利用して簡単な個人用のチャットシステムを作ってみたのでここに記載する。

# LangChain とは？

LangChain[^1]は、大規模言語モデル（LLM）を活用したアプリケーション開発を簡単にするためのフレームワークである。OpenAI の GPT や Anthropic Claude など様々な LLM を統一的なインターフェースで扱えるように設計されており、複雑な AI アプリケーションを効率的に構築できる。
...
これにより、後から他の環境で同じ依存を再現できます。

## OpenAI での API キー設定

言語モデルは複数あり、どれを選択しても良いのですが、今回は`openai`を利用します。

OpenAI API を使うためには「API キー」と呼ばれる秘密の文字列（トークン）が必要です。これは OpenAI のサービスを認証・利用するためのパスワードのようなものです。以下で順を追って説明します。

### OpenAI API キーとは

OpenAPI API キーとは、 OpenAI のサーバーに「誰がリクエスト（問い合わせ）を送ってきているか」を証明するためのトークンです。

API キーがないと、OpenAI のモデル（GPT-4 など）を呼び出すことができません。
...
ログイン後、画面左のサイドバーに「API keys」という項目があるのでクリック。
...
画面に表示された文字列（`sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx` のような長い文字列）をコピー。
...
回答: 現在の日本の首都は東京ですが、首都を移転する場合は慎重に検討する必要があります。移転先を選ぶ際には、政治的、経済的な影響や地理的条件、文化的背景などを考慮する必要があります。具体的な移転先をお勧めすることは難しいですが、移転を検討する場合は国の全体像を含めて総合的に検討することが大切です。
...
4. 現在の日本の首都は東京ですが、首都を移転する場合は慎重に検討する必要があります。移転先を選ぶ際には、政治的、経済的な影響や地理的条件、文化的背景などを考慮する必要があります。具体的な移転先をお勧めすることは難しいですが、移転を検討する場合は国の全体像を含めて総合的に検討することが大切です。
...
と言うような感じで今回は Langchain によるチャットシステムの作成を行ってみたが、ここで使用したもの以外にも他のモデルを利用したりするなどといった機能も満載なので、今後も何か試していきたいと思う。
