...
​
所属するグループに関しては各自の環境に応じて設定すること。
​
ロールやポリシーに関しては、アクセス許可の所で設定する。

今回は説明用として FullAccess を利用しますが、最小権限で行うのが望ましいので、CDK の運用時には必要最低限な許可を付与して下さい。
​
​<!-- 設定方法を図示する？ -->
...
​
IAM ユーザーを作成した後、ユーザーのアクセスキーとシークレットアクセスキーをダウンロードできるので、設定のためにダウンロードする。
​
