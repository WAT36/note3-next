...
Next.js のレンダリング方式について、思い出しきれなくなったため今一度各レンダリング方式の特徴と使い分けを整理しようと思う。
...
今回は、ハンズオンを通してこれらをそれぞれ実行して確認してみます。
...
- **SEO**: 弱い。クローラーが JavaScript を実行しないと内容を読めない
- **更新頻度**: リアルタイム向け
- **コスト**: 低い。サーバー負荷が少ない
...
開発モード(`npm run dev`)では、すべてのページが動的に生成されます。本番環境での動作を確認するには、必ず `npm run build` してから `npm start` で確認しましょう。

## ISR が動作しない場合

開発モードでは ISR は動作しません。必ず本番ビルド後に確認してください。
...
App Router では、デフォルトでサーバーコンポーネントです。`useState`, `useEffect`などの React Hooks を使う場合は、ファイルの先頭に `'use client'` を記述してクライアントコンポーネントにする必要があります。
