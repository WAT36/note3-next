...
coverImage: ""
date: "2026-01-05T01:25:26.000Z"
updatedAt: "2026-01-05T01:25:26.000Z"
tag: ["AI"]
author:
...
以前の記事で Vector Database についてを書いたが、今回はそれを活用した**RAG(Retrieval-Augmented Generation)**についてを記載していく。
...
1. **知識の鮮度問題**: 学習データの期限(知識カットオフ)以降の情報を知らない
2. **ハルシネーション(幻覚)**: もっともらしい嘘をついてしまうことがある
3. **専門知識の不足**: 企業固有の情報や専門的な内容は学習していない
...
> ↓
>
> 1.  質問に関連する情報を Vector Database から検索
>     ↓
...
- RAG: Vector DB から決算資料を検索 → 「2024 年 Q3 の売上は 150 億円でした。前年比 15%増となっています。(参照: 2024 年 Q3 決算報告書)」

## RAG の主な特徴とメリット

1. 最新情報への対応

Vector Database に最新の文書を追加するだけで、LLM が最新情報に基づいて回答できます。モデルの再学習は不要です。
...
これらを Vector Database に格納することで、LLM が専門家のように回答できます。
...
長い文書をそのまま Vector Database に入れると検索精度が落ちるため、適切なサイズ(通常 100〜500 トークン)に分割します。
...
それでは、前回の Vector Database の知識を活用して、実用的な RAG システムを構築していきましょう。

## 前提条件

このハンズオンは、前回の「Vector Database 完全ガイド」の環境構築が完了していることを前提としています。
...
文書をチャンクに分割して Vector Database に格納するスクリプト`data_preparation.py`を作成します。
...
# sample_docs.txtを作成(上記の内容をコピー)# スクリプトを実行
...
============================================================
✓ 完了! 8個の文書をVector Databaseに格納しました
  保存場所: ./chroma_db
============================================================
...
2. **retrieve()**: Vector DB から関連文書を検索
...
✓ 2件の関連文書を取得しました
  ...
  内容: RAGは、大規模言語モデル(LLM)の回答生成能力と情報検索を組み合わせた技術です。ユーザーからの質問に対して、まずVector Databaseから関連する文書を検索し、その情報をコンテキストとして...
...
============================================================
回答:
RAG（Retrieval-Augmented Generation）とは、大規模言語モデル（LLM）の回答生成能力と情報検索を組み合わせた技術です。この技術は、ユーザーからの質問に対してまず関連する文書をVector Databaseから検索し、その情報をコンテキストとしてLLMに渡します。これにより、より正確で根拠のある回答を生成することができます。

具体的には、RAGは以下のように動作します：
1. ユーザーの質問を受け取る。
2. Vector Databaseから関連する文書を検索する。
