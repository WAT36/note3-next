...
GraphQL がよくわかっていなかったため、簡単な実践を兼ねて調べてみました。

まとめると長くなってしまったため、紹介編（本記事）と実践編（次記事）に分けて記載する。

# GraphQL とは

GraphQL[^1] は、API のためのクエリ言語であり、既存のデータに対してクエリを実行するためのランタイムです。Facebook で開発され、2015 年にオープンソース化されました。

# GraphQL の特徴

GraphQL の特徴をいくつか列挙します。
...
クライアントが必要なデータのみを指定して取得できます。これにより、Over-fetching や Under-fetching の問題を解決します。以下に例を記載します。
...
GET /api/users
// 結果
//[({ id: 1, name: "田中太郎" }, { id: 2, name: "佐藤花子" })]

// 各ユーザーの投稿数も知りたい → 追加のリクエストが必要
...
既存のクエリを壊さずに新しいフィールドを追加できるため、API のバージョニングが不要になります。
...
- 複数のセットが欲しい場合は、何度も注文する必要あり

**GraphQL = バイキングレストラン**

- 好きな料理を自由に組み合わせて注文できる
- 不要な料理は除いて、必要な分だけ注文可能
- 一度の注文で、複数の料理を まとめて頼められる
...
Subscription はデータの変更をリアルタイムで監視・受信する操作です。（クライアント側から WebSocket 接続を始める仕組み）
...
とりあえず本記事では、GraphQL の特徴を調べて書いてみました。

次の記事では、実践としてハンズオンを記載する。
