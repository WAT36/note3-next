...
ChatGPT や Cursor と外部データソースをつなぐ新しい仕組みですが、実際にどう動いているのか気になったので、今回は自分のローカル環境で MCP サーバーを立て、Cursor から使ってみたので、今回はそれについて書いていきます。
...
最初に記載しておきますが、今回のプロジェクト構成は以下の通りです。
...
今回は簡単で基本的な例をお見せしましたが、もう少し実用的な外部サービスや外部の API との連携などもやりたいと感じているので、今後広げて実践していきたい。

最後に注意点として、MCP サーバーは LLM が直接 API キーを扱わないため安全ですが、社内システムとつなぐ場合は**認可設定を慎重に**しましょう。
