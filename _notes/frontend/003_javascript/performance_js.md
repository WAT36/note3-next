...
Worker はグローバルオブジェクトとして**WorkerGlobalScope**をもち、Worker の内部で利用することができます。

WorkerGlobalScope の API を以下に示します。

| プロパティ | 説明                                                |
| :--------- | :-------------------------------------------------- |
| self       | WorkerGlobalScope オブジェクト自身                  |
| location   | Worker が作成された時の WorkerLocation オブジェクト |
| navigator  | WorkerNavigator オブジェクト                        |

| メソッド        | 説明                                                   |
| :-------------- | :----------------------------------------------------- |
| close()         | Worker をクローズ状態にする                            |
| importScripts() | 外部スクリプトファイルやライブラリをワーカーに組み込む |
| postMessage()   | Worker にメッセージを送信する                          |

| イベントハンドラ | 説明                               |
| :--------------- | :--------------------------------- |
| onerror          | エラーが発生した場合に発火する     |
| onoffline        | オフラインになった場合に発火する   |
| ononline         | オンラインになった場合に発火する   |
| onmessage        | メッセージを受信した場合に発火する |

このうち、WorkerLocation オブジェクトの API は以下の通り。

| プロパティ | 説明                           |
| :--------- | :----------------------------- |
| href       | このオブジェクトが表す完全 URL |
| protocol   | プロトコル                     |
| host       | ホスト                         |
| hostname   | ホスト名                       |
| port       | ポート番号                     |
...
Worker でエラーが発生した場合は ErrorEvent が発生する。そのプロパティは以下の通りになる。

| プロパティ | 説明                                         |
| :--------- | :------------------------------------------- |
| filename   | エラーの原因となった Worker スクリプトの名前 |
| lineno     | エラーが発生した行番号                       |
| message    | エラーの説明                                 |
