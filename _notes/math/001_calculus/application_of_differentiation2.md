...
曲線 y=f(x)上の点 A(a,f(a))を境として曲線の凹凸の状態が入れ替わる時、この点 A を**変曲点**という。

> 関数 f(x)は２回微分可能であって、f’’(x)は連続であるとする。このとき、f’’(a)=0 であって、x=a を境にして f’’(x)の符号が変われば、点 A(a,f(a))は曲線 y=f(x)の変曲点である。
