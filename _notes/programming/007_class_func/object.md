...
Javascript でのオブジェクトは、上記のような方法で記載する。

連想配列と基本記法は同じだが、オブジェクトは配列的な意味合いではない点が連想配列とは異なってくる。

オブジェクトは、オブジェクト自体が 1 つのモノであり、中に含まれる要素は、オブジェクトの特性や動作を表すために存在している。

オブジェクトは、プロパティ（キーと値のセット）と、メソッドから構成される。
